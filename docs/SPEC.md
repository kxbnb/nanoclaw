...
The `.env` file is copied to `data/env/env` (run `cp .env data/env/env` after changes) and mounted into the container, then sourced by the entrypoint script. This workaround is needed because Apple Container loses `-e` environment variables when using `-i` (interactive mode with piped stdin).
